...
# Matching between t-stats (correlation)
```{r}
...
RdBu.pal = circlize::colorRamp2(seq(0, 0.5, length.out = 7), pals::brewer.purples(7))
CC = cor(McLean.tstat, MtSinai.tstat)
# CC = t(McLean.tstat) %*% MtSinai.tstat
rownames(CC) = colnames(CC) = cts
CC[CC < 0] = 0
ct = intersect(names(resDE$MtSinai$table$PhenotypeSZ), names(resDE$McLean$table$PhenotypeSZ))
pdf("~/results/figures/pairwise_cor_masked_logumi_baseline_full_filter_extended_cell_filtering.pdf", width = 8)
Heatmap((CC), cluster_rows = F, cluster_columns = F, name = "Correlation (truncated)", col = RdBu.pal, rect_gp = gpar(col = "black"), row_names_side = "left", row_names_gp = gpar(col = colors[rownames(CC)]), column_names_gp = gpar(col = colors[colnames(CC)]))
dev.off()
...
pdf(file.path(figures.folder, "Pi1_logumi_baseline_full_filter_extended_cell_filtering.pdf"))
plot(gg)
dev.off()
...
pdf(file.path(figures.folder, "DE_vs_bulk_logumi_baseline_full_filter_extended_cell_filtering.pdf"))
plot(gg)
dev.off()
...
pdf(file.path(figures.folder, "NumDysregGenes_logumi_baseline_full_filter_extended_cell_filtering.pdf"), width = 8, height = 5)
...
```{r}
genes = read.table("~/GenesToVis5_18_2021.csv")[, 1]
genes = intersect(genes, intersect(names(PEC.tstat), names(CMC.tstat)))

X = DE.new$DE.sc[genes, ]
perm = order(fast_row_sums(X), decreasing = T)
Y = cbind(PEC.tstat[genes], CMC.tstat[genes])
colnames(Y) = c("PEC", "CMC")
RdBu.pal = circlize::colorRamp2(seq(-3, 3, length.out = 7), rev(pals::brewer.rdbu(7)))

pdf(file.path(figures.folder, "selected_genes.pdf"), width = 6, height = 32)
...
XU = DE.new$DE.sc
XU[XU < 0] = 0
pdf(file.path(figures.folder, "top_up.pdf"), width = 6, height = 32)
plot.top.k.features(XU)
dev.off()
...
## Load annotations
```{r}
Up.enrichment.GS = assess.genesets(Up.genes, SZ.genes, nrow(pb.logcounts), correct = "local")
Down.enrichment.GS = assess.genesets(Down.genes, SZ.genes, nrow(pb.logcounts), correct = "local")
...
pdf(file.path(figures.folder, "DE_FunCat_enrichment_meta_filtered_logumi_baseline_full_filter_extended_cell_filtering_neuro.pdf"), width = 14, height = 7)
par(mar=c(0,150,0,0))
