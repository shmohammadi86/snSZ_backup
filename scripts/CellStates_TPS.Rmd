...
```{r}
sample.df = data.frame(Label = pb.logcounts$ID, Phenotype = pb.logcounts$Phenotype, TPS = scale(pb.logcounts$TPS), SZTR = scale(pb.logcounts$A17.signature))
...
pdf(file.path(figures.path, "SZTR_vs_phenotype.pdf"))
plot(gg)
dev.off()
