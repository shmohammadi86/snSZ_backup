                        ...
                        annotatePval = NULL, annotateTop = TRUE, annotateLabels = NULL,...) 
{
  ...
  if(!is.null(annotateLabels)) {
    topHits = subset(d, Labels != "")
    par(xpd = TRUE)
    with(topHits, text(pos, -log10(P), Labels, col = Labels.col, cex = 0.3, srt = 90), 
         ...)
  }
...
manhattan.compute <- function (x, chr = "CHR", bp = "BP", p = "P", snp = "SNP", col = c("gray10", 
                                                                                        "gray60"), chrlabs = NULL, suggestiveline = -log10(1e-05), 
                               genomewideline = -log10(5e-08), highlight = NULL, logp = TRUE, 
                               annotatePval = NULL, annotateTop = TRUE, annotateLabels = NULL,...) 
{
...
manhattan.plot <- function (d, chr = "CHR", bp = "BP", p = "P", snp = "SNP", col = c("gray10", 
  ...
  if(!is.null(annotateLabels)) {
    topHits = subset(d, Labels != "")
    topHits$base = rep(0, nrow(topHits))
    par(xpd = TRUE)
    with(topHits, text(pos, -log10(P)+(str_length(Labels)*0.17), Labels, col = Labels.col, cex = 0.3, srt = 90), ...)
    #         for( i in 1:nrow(topHits)) {
    # 	       plot(x = c(topHits$pos[i], topHits$pos[i]), y = c(0, topHits$logp[i]), type="l",lwd = 2,  col = topHits$Labels.col[i])
    #         }
    
    with(topHits, segments(x0 = pos, y0 = 0, y1=-log10(P), lwd = 1,  col = Point.col), ...)
